...
Lessons are ranked by contextual similarity, with a confidence ≥0.5 gate at creation. Every decision records which lessons it was shown; `review_lessons` compares exposed/unexposed mean regret and can retire a lesson after at least three uses. These are observational comparisons, not proof of causation; recurrence and relevance-cutoff gates remain pending. The top **Cómo decide y aprende** strip shows forecast, adaptation, cases sent (not confirmed use) and descriptive regret history. Click a stage to open evidence in a dialog without scrolling the page. The learning view includes lesson retire/restore controls; `/api/learning` exposes the same data.
