...
Object.assign(I18N.en,{
  ...
  adReflex:'Who decides?',adReflexCentral:'Central (HappyRobot)',adReflexWaiting:'Jev reflex in shadow: recorded beside each decision, never acts',adReflexSub:(a,ms,route)=>`Jev (shadow) · agrees ${a}% · ${ms} ms · ${route==='reflex'?'would have acted':'would have escalated'}`,adReflexGrade:(g)=>g==null?'':` · cost ${g>0?'+':''}${g} vs Central`,pmReflex:'Jev reflex (shadow)',pmReflexCell:(r)=>`${r.route==='reflex'?'would act':'would escalate'} · ${Math.round((r.agreement??0)*100)}% same · ${r.latency_ms} ms${r.grade?` · regret ${r.grade.regret}`:''}`,
