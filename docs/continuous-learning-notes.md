...
| Tool-mediated retrieval (ReAct/Toolformer style) | The agent (or a step before it) calls a tool for memory | Judgement over which precedents apply | Callable *Obtener experiencia* workflow: reads the brief, judges applicability, returns validated mission fields with provenance; deterministic fallback |
