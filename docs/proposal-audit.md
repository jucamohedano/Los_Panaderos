...
| Cross-incident case retrieval | **Implemented.** Up to three graded cases, belief-only signature, distance cutoff, current-incident exclusion. Versioned labels explain shared context and differences; explicitly truncated/invalid grades are excluded. | Add stronger distribution-shift tests, bounded text lengths, resource compatibility and context diversity. Labels currently explain numeric ranking rather than change it. Legacy grades without a truncation flag remain eligible. |
...
| Clean, no-page-scroll learning/adaptation view | **Implemented.** Adaptación uses four newcomer questions above maps. Experience/results have separate dialog views, labelled case comparisons and chart, optional technical details and rule controls. ES/EN, responsive layout, focus/disclosure preservation, delayed responses and replay/reset were browser-tested. | Preserve honest states. A completed replan does not itself mean a forecast check passed. Synthetic UI values do not demonstrate learning. |

The [adaptation and experience guide](adaptation-and-learning.md) explains the
current implementation, labels, two meanings of replay and design rationale.
